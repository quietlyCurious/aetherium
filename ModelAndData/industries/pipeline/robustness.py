#!/usr/bin/env python3
"""Robustness check for the pipeline detectors: do they still find each scenario,
...
detector on each copy (nothing is written to public/data/), and counts:
  ...
  extra    — a detector fired on an asset no attention item covers
    ...
    python3 ModelAndData/industries/pipeline/robustness.py [N_SEEDS]
...
spec = importlib.util.spec_from_file_location('pipeline_explain', os.path.join(HERE, 'explain.py'))
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for s in range(1, n + 1):
            ...
            os.makedirs(d)
            generate(seed, d)
            pack, out, report, _ = run_pack(W.REPO, 'pipeline', W.DETECTORS, pack_dir=d, write=False)
